...
import { FriendRequestModel } from "../../models/freindRequest";
...
sendFriendReq = async (req: Request, res: Response): Promise<Response> => {
  ...
  if (authUser._id.toString() === to) {
    throw new ApplicationException("you can't send friend request to your self", 400);
  }

  const friend = await this.userModel.findById({
    id: to
  });
